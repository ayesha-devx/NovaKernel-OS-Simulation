...
import { FiTrendingUp, FiActivity, FiClock, FiCheckCircle, FiCpu } from 'react-icons/fi';
...
const Scheduler = () => {
  ...
  const metricCards = [
    { label: 'Avg_Waiting_Time', value: `${schedulerMetrics.avg_waiting_time}s`, icon: <FiClock />, color: '#FFC857', glow: 'rgba(255, 200, 87, 0.3)' },
    { label: 'Avg_Turnaround', value: `${schedulerMetrics.avg_turnaround_time}s`, icon: <FiTrendingUp />, color: '#D900FF', glow: 'rgba(217, 0, 255, 0.3)' },
    { label: 'CPU_Utilization', value: `${schedulerMetrics.cpu_utilization}%`, icon: <FiActivity />, color: '#00D1FF', glow: 'rgba(0, 209, 255, 0.3)' },
    { label: 'Units_Completed', value: schedulerMetrics.throughput, icon: <FiCheckCircle />, color: '#00FF9D', glow: 'rgba(0, 255, 157, 0.3)' },
  ];
      ...
      <div className="relative min-h-screen">
        ...
        >
            ...
            <motion.div variants={itemVariants} className="relative group">
                <div className="absolute inset-0 bg-gradient-to-r from-primary/15 via-secondary/5 to-transparent rounded-[2.5rem] blur-2xl opacity-50 group-hover:opacity-80 transition-opacity" />
                   ...
                   <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-10 relative z-10">
                     <div className="space-y-4">
                       <div className="flex items-center gap-6">
                          <div className="w-16 h-16 rounded-2xl bg-secondary/20 border border-secondary/30 flex items-center justify-center neon-border-secondary">
                              <FiCpu className="text-secondary neon-text-cyan" size={36} />
                          </div>
                          <div>
                            <h1 className="text-5xl font-black text-white font-orbitron tracking-tighter neon-gradient-text uppercase leading-none mb-1">SCHEDULER</h1>
                       ...
                       <p className="text-text/40 text-xs font-bold uppercase tracking-widest max-w-2xl leading-loose ml-22">
                     ...
                     <div className="grid grid-cols-2 gap-6 w-full lg:w-auto">
                        {metricCards.map((card, idx) => (
                          <motion.div 
                            ...
                            className="relative glass-premium bg-white/[0.02] border border-white/5 px-8 py-5 rounded-[2rem] overflow-hidden group/card shadow-[0_0_20px_rgba(0,0,0,0.2)]"
                          >
                             <div className="absolute top-0 right-0 p-4 opacity-5 group-hover/card:opacity-20 transition-opacity">
                                {card.icon}
                             </div>
                             <div className="flex flex-col gap-1">
                                <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] font-orbitron">{card.label}</span>
                                <div className="flex items-baseline gap-2">
                                    <p className="text-2xl font-black font-orbitron text-white tracking-tight">{card.value}</p>
