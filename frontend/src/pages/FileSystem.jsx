  ...
  return (
    <DashboardLayout title="FILE SYSTEM">
      <div className="relative z-10 space-y-10 pb-12">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-r from-cyan/20 via-primary/5 to-transparent rounded-[2.5rem] blur-2xl opacity-50 group-hover:opacity-80 transition-opacity" />
          <div className="relative glass-premium rounded-[2.5rem] p-10 border border-white/10 overflow-hidden shadow-[0_0_50px_rgba(0,209,255,0.05)]">
             ...
             <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 relative z-10">
               <div className="space-y-4">
                 <div className="flex items-center gap-5">
                    <div className="w-14 h-14 rounded-2xl bg-cyan/20 border border-cyan/30 flex items-center justify-center neon-border-cyan">
                        <LuHardDrive className="text-cyan neon-text-cyan" size={32} />
                    </div>
                    <div>
                      <h1 className="text-5xl font-black text-white font-orbitron tracking-tighter neon-gradient-text uppercase leading-none mb-1">FILE SYSTEM</h1>
                      <div className="flex items-center gap-3">
                          <span className="w-2 h-2 rounded-full bg-success shadow-[0_0_10px_#00FF9D] indicator-pulse" />
                          <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] font-orbitron">Virtual_NVME_Storage_Active</p>
                      </div>
                    </div>
                 </div>
               ...
               <div className="flex items-center gap-6 px-8 py-4 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-xl">
                  <div className="flex flex-col items-end gap-1">
                      ...
                      <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-mono font-black text-white uppercase">NVME_X4_LINK_ACTIVE</span>
                      </div>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="glass-premium border border-white/5 rounded-[2.5rem] p-8 relative overflow-hidden group">
              ...
              <h3 className="text-[10px] font-black text-white/80 mb-8 flex items-center gap-3 uppercase tracking-[0.3em] font-orbitron relative z-10">
                ...
                <table className="w-full text-left border-separate border-spacing-y-3">
                   <thead>
                      <tr className="text-[9px] text-slate-500 font-black uppercase tracking-[0.2em] font-orbitron">
                         ...
                         <th className="px-6 py-2">Blocks</th>
                         <th className="px-6 py-2">Created</th>
                         <th className="px-6 py-2 text-right">Perms</th>
                      </tr>
                   </thead>
                   <tbody>
                      {directory.map((inode) => (
                        <tr key={inode.inode_id} className="bg-white/[0.03] border border-white/5 hover:bg-white/[0.08] hover:neon-border-cyan transition-all duration-300 group rounded-xl">
                           <td className="px-6 py-4 text-xs font-mono text-cyan font-bold">#{inode.inode_id.toString().padStart(3, '0')}</td>
                           <td className="px-6 py-4 text-xs font-black text-white group-hover:text-cyan transition-colors uppercase tracking-wider">{inode.filename}</td>
                           <td className="px-6 py-4 text-xs text-slate-400 font-space">{inode.size} MB</td>
                           <td className="px-6 py-4 text-xs font-mono text-slate-500">[{inode.block_count}]</td>
                           <td className="px-6 py-4 text-[10px] text-slate-500 font-mono">{new Date(inode.created_at).toLocaleTimeString()}</td>
                           <td className="px-6 py-4 text-[10px] font-mono text-green/80 text-right">
          ...
          <div className="space-y-8">
            ...
            {/* FS Logs */}
            <div className="glass-premium border border-white/5 rounded-[2.5rem] p-8 relative overflow-hidden">
        ...
        {selectedFile && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="bg-surface/95 backdrop-blur-3xl border border-white/10 rounded-[3rem] w-full max-w-2xl overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] relative z-10 neon-border-cyan"
            >
              <div className="absolute inset-0 scanline-overlay opacity-10 pointer-events-none" />
              <div className="p-8 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-cyan/10 to-transparent relative z-10">
                 <div className="flex items-center gap-6">
                    <div className="w-16 h-16 rounded-[1.5rem] bg-cyan/10 flex items-center justify-center text-cyan border border-cyan/20 shadow-[0_0_20px_rgba(0,209,255,0.2)]">
                       <LuFileText size={32} />
                    </div>
                    <div>
                       <h2 className="text-2xl font-black text-white uppercase tracking-tighter font-orbitron">{selectedFile.filename}</h2>
                 ...
                 <button onClick={() => setSelectedFile(null)} className="p-3 hover:bg-white/5 rounded-2xl text-slate-500 hover:text-white transition-all border border-transparent hover:border-white/10">
                    <LuX size={24} />
                 </button>
              </div>
              ...
              <div className="p-10 relative z-10">
                 <div className="mb-6 flex items-center justify-between">
                    ...
                    <div className="flex gap-3">
                       {isEditing ? (
                         <button onClick={saveFile} className="flex items-center gap-2 px-5 py-2 bg-green text-slate-950 text-[10px] font-black rounded-xl hover:bg-white transition-all shadow-[0_0_20px_rgba(0,255,157,0.4)]">
                            <LuSave size={14} /> COMMIT_TO_DISK
                         </button>
                       ) : (
                         <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 px-5 py-2 bg-white/5 text-white text-[10px] font-black rounded-xl hover:bg-white/10 transition-all border border-white/10 hover:border-cyan/50">
                            OVERRIDE_FILE
                         </button>
                       )}
                    </div>
                 ...
                 <div className="relative group/textarea">
                    <textarea 
                      ...
                      readOnly={!isEditing}
                      className={`w-full h-80 bg-black/40 border border-white/5 rounded-3xl p-8 text-sm font-mono text-cyan/80 focus:ring-1 focus:ring-cyan/50 outline-none resize-none transition-all custom-scrollbar ${isEditing ? 'border-cyan/50 bg-black/60 shadow-[inset_0_0_30px_rgba(0,209,255,0.05)]' : ''}`}
              ...
              <div className="p-8 bg-black/40 border-t border-white/5 flex justify-between items-center text-[10px] text-slate-500 font-mono">
