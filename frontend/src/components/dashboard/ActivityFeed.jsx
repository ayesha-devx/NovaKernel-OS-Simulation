...
const ActivityFeed = () => {
  ...
  return (
    <div className="glass-premium rounded-[2.5rem] border border-white/10 flex flex-col h-full overflow-hidden relative group shadow-[0_0_40px_rgba(157,0,255,0.05)]">
      ...
      <div className="px-8 py-6 border-b border-white/5 bg-white/[0.02] flex items-center justify-between relative z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-[10px] font-black text-text/80 uppercase tracking-[0.4em] font-orbitron">Live_Activity_Flux</h3>
        ...
        <div className="flex items-center gap-3 px-4 py-1.5 glass-premium bg-primary/10 rounded-full border border-primary/30">
            <span className="w-2 h-2 rounded-full bg-primary shadow-[0_0_12px_rgba(157,0,255,1)] indicator-pulse" />
            <span className="text-[9px] text-primary font-black tracking-[0.2em] font-orbitron">SYNC_ACTIVE</span>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto p-6 space-y-3 relative z-10 scrollbar-hide">
        <AnimatePresence mode="popLayout">
          {activities.length > 0 ? activities.map((activity, i) => (
            <motion.div 
              ...
              className="flex items-start gap-5 p-5 rounded-2xl border border-transparent hover:border-white/5 transition-all group/item relative overflow-hidden"
            >
      ...
      <motion.button 
        ...
        className="p-6 text-[10px] font-black text-primary hover:text-white transition-all uppercase tracking-[0.4em] border-t border-white/5 font-orbitron flex items-center justify-center gap-3 group/btn"
      >
